 ...
 * @since 2015.3.13
 */
class dollAction extends dollBaseAction {
	public $mopenid;
	public $nickname;
	public $headimgurl;
	public $M;
	public function __construct() {
		...
		$this->assign ( 'title', '' );
		$this->assign ( 'url', APP_PATH . 'index.php/doll/index/sourceopenid/' . $this->mopenid );
	}
	...
	public function index() {
		...
		$user = $this->M->getData ( "sys_member", "openid='{$this->mopenid}'" );
		$info = $this->M->getData ( 'newdoll_user', "openid='{$this->mopenid}'", TRUE );
		$this->assign ( "count", $info ['lastlotterytimes'] );
		if ($user) {
			$this->assign ( "fenshi", 'yes' );
		} else {
			$this->assign ( "fenshi", 'no' );
		}
		...
		$this->display ( 'wawaji/index.html' );
	}
	public function rules() {
		$this->display ( 'wawaji/rules.html' );
	}
	public function index1() {
		$this->display ( 'wawaji/index1.html' );
	}
	...
	public function play() {
		...
		$num = $this->M->getData ( 'newdoll_user', "openid='{$openid}'", true );
		$nums = $num ['lastlotterytimes'];
		$orderList = $this->M->getOrderList ();
		foreach ( $orderList as $k => $v ) {
			if ($this->mopenid == $v ['openid']) {
				$order = $k + 2;
			...
			if ($openid == $first_openid) {
				switch ($status) {
					...
					case 1003 :
						...
						ajaxReturn ( "http://vm.tbswx.com/pay/mobile/doll/index.php?openid={$openid}&deviceid=1003&nums={$nums}", 'success', 1 );
						break;
					case 1004 :
						$this->gogame ();
						// redirect("http://vm.tbswx.com/pay/mobile/doll/index.php?openid={$openid}&deviceid=1003");
						ajaxReturn ( "http://vm.tbswx.com/pay/mobile/doll/index.php?openid={$openid}&deviceid=1004&nums={$nums}", 'success', 1 );
						break;
					default :
						$this->upOrderTime ();
						ajaxReturn ( $order, '机器繁忙！', 0 );
	...
	public function gogame() {
		$sql = "update newdoll_user set is_play='2' where openid='{$this->mopenid}'";
		// $info['is_play'] = '2';
		$this->M->bysql ( 'newdoll_user', $sql );
		// $this->M->upData('newdoll_user', "openid='{$this->mopenid}'", $info);
	}
	...
	public function getLottery() {
		...
		if ($userinfo ['lastlotterytimes'] > 0) {
			...
			// 控制用户只能中奖一次
			$temp = $this->M->getData ( 'newdoll_lottery_log', "openid='{$this->mopenid}' and type != 0" );
			if ($temp) {
				$rand = 0;
			}
	...
	public function upAddress() {
		...
		$this->M->upData ( 'newdoll_lottery_log', array (
				'openid' => $this->mopenid 
		), $info );
	}
	...
	private function chkGift($type) {
		...
		// 获取当日已领取礼品数量
		$todayCount = $this->M->getCount ( 'newdoll_lottery_log', ' type = ' . $type . ' AND ctime > ' . strtotime ( 'today' ) );
		$timeinfo = getdate ();
		$mday = $timeinfo ['mday'];
		// 获取当前礼品的当日配额
		$conf = $this->M->getData ( 'newdoll_lottery_conf', array (
				'gtype' => $type,
				'mday' => $mday 
		), TRUE );

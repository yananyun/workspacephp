...
class Image {
	 ...
	 * @return void +----------------------------------------------------------
	 */
	static function thumb($image, $thumbname, $type = '', $maxWidth = 200, $maxHeight = 500, $interlace = true) {
		// 获取原图信息
	 ...
	 *        	字符串
	 * @param string $size
	 *        	图像大小 width,height 或者 array(width,height)
	 * @param string $font
	 *        	字体信息 fontface,fontsize 或者 array(fontface,fontsize)
	 * @param string $type
	 *        	图像格式 默认PNG
	 * @param integer $disturb
	 ...
	 * @return string +----------------------------------------------------------
	 */
	static function buildImageVerify($length = 4, $mode = 2, $type = 'png', $width = 48, $height = 22, $verifyName = 'verify') {
		load_sys_class ( "String" );
	...
	static function GrabImage($url, $filename = "") {
		 ...
		 * //如果取值有问题，则尝试三次
		 * $cnt=0;
		 * while($cnt < 5 && ($stream=@file_get_contents($url, false, $context))===FALSE){
		 * $cnt++;
		 * }
		 */
		
		$stream = curl_file_get_contents ( $url );
	 ...
	 * @return unknown
	 */
	static function GetRomoteThumb($file) {
		set_time_limit ( 0 );
		$path = '';
		if ($file) {
			$basepath = str_replace ( 'lib', '', dirname ( __FILE__ ) );
			$localfile = $basepath . '/uploads/orginal/' . basename ( $file );
			$filename = '/uploads/thumbnail/' . basename ( $file );
			$thumbfile = $basepath . $filename;
			
			$flag = self::GrabImage ( $file, $localfile );
			if ($flag) {
				$handle = self::thumb ( $localfile, $thumbfile, 'jpg', 200, 150 );
				if ($handle) {
					$path = $filename;
				}
			}

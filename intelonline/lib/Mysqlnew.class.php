 ...
 */
class Mysqlnew {
	private static $connect_rw; // 数据主库连接
	private static $connect_ro; // 数据从库连接
	...
	public function __construct($dbConfig) {
		...
		if (! self::$connect_rw) {
			try {
				self::$connect_rw = self::connect ( $dbConfig ['Master'], $dbConfig ['DBname'] ? $dbConfig ['DBname'] : $dbConfig ['Master'] ['DBname'] );
		...
		if (! self::$connect_ro) {
			if (is_array ( $dbConfig ['Slave'] ) && $dbConfig ['Slave']) { // 当从库不为空的时候
				...
				if ($link_ro) {
					try {
						self::$connect_ro = self::connect ( $link_ro, $dbConfig ['DBname'] ? $dbConfig ['DBname'] : $link_ro ['DBname'] );
					} catch ( PDOException $e ) {
						echo '从库连接：' . $e->getMessage ();
						exit ();
					}
				} else {
					self::$connect_ro = self::$connect_rw;
				}
			}
		}
		/*
	...
	static private function _query($sql) {
		if (! self::$connect_ro) { // 判断从库是不是可用的
			self::$connect_ro = self::$connect_rw; // 如果不可用则用主库
		}
		self::$sql_str = $sql;
		try {
			$ret = self::$connect_ro->query ( $sql );
		} catch ( PDOException $e ) {
			return false;
		}
		if ($ret) {
	 ...
	 */
	static private function _exec($sql, $insert = false) {
		self::$sql_str = $sql;
		try {
			$ex = self::$connect_rw->exec ( $sql );
		} catch ( PDOException $e ) {
			return false;
		}
		if ($insert) {
			$ex = self::$connect_rw->lastInsertId ();
		}
		return $ex;
	}
	/**
	...
	public function beginTransaction() {
		self::$connect_rw->setAttribute ( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
		self::$connect_rw->beginTransaction ();
	}
	/**
	 * 事务回滚结束标记
	 */
	...
	public function commit() {
		self::$connect_rw->commit ();
	}
	/**
	 * 事务开始回滚标记
	 */
	public function rollBack() {
		self::$connect_rw->rollBack ();
	}
	/**
	 * 格式化排序依据,将多个空格缩减为一个
	 *

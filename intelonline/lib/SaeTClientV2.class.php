...
class SaeTClientV2 {
	 ...
	 *        	返回结果的页码。
	 * @param int $count
	 *        	单页返回的最大记录数，最多返回200条，默认50。可选。//接口写错,最大只能返回100
	 * @param int $since_id
	...
	function bilateral_timeline($since_id = 0, $max_id = 0, $count = 20, $page = 1, $base_app = 0, $feature = 0, $trim_user = 0) {
		$par = array ();
		$par ['since_id'] = $since_id;
		$par ['max_id'] = $max_id;
		$par ['count'] = $count;
		$par ['page'] = $page;
		$par ['base_app'] = $base_app;
		$par ['feature'] = $feature;
		$par ['trim_user'] = $trim_user;
		return $this->oauth->get ( 'statuses/bilateral_timeline', $par );
	}
	 ...
	 *
	 */
	function followers_active($uid, $count = 200) {
		$param = array ();
		$this->id_format ( $uid );
		$param ['uid'] = $uid;
		$param ['count'] = $count;
	 ...
	 *        	要修改的学校信息。格式：array('key1'=>'value1', 'key2'=>'value2', .....)。
	 *        	支持设置的项：
	 ...
	 *        	要修改的职业信息。格式：array('key1'=>'value1', 'key2'=>'value2', .....)。
	 *        	支持设置的项：
	 ...
	 *        	返回的记录条数，默认为10。
	 * @param
	 *        	int type 学校类型，0：全部、1：大学、2：高中、3：中专技校、4：初中、5：小学，默认为0。选填
	 * @return array
	 */
	 ...
	 *        	分类，可选参数，返回某一类别的推荐用户，默认为 default。如果不在以下分类中，返回空列表：<br />
	 *        	- default:人气关注
	 *        	- ent:影视名星
	 *        	- hk_famous:港台名人
	 *        	- model:模特
	...
	// 短链-->点击数
	function short_url($url_short) {
		$params = array ();
		$params ['url_short'] = $url_short;
		return $this->oauth->get ( 'short_url/clicks', $params );
	}
	
	// 短链-->获取一个短链接点击的地区来源和数量
	function short_url_locations($url_short) {
		$params = array ();
		$params ['url_short'] = $url_short;
		$this->oauth->get ( 'short_url/locations', $params );
	}
	
	/**
	 * 批量获取用户的粉丝数、关注数、微博数
	 *
	 * <br />对应API：{@link https://api.weibo.com/2/users/counts.json}
	 *
	 * @access public
	 * @param
	 *        	string uids string 需要获取数据的用户UID，多个之间用逗号分隔，最多不超过100个
	 * @return array
	 */
	function ucounts($uids) {
		$params = array ();
		$params ['uids'] = $uids;
	...
	/**
	 * 批量获取指定微博的转发数评论数 2012-06-06
	 *
	 * <br />对应API：{@link https://api.weibo.com/2/statuses/count.json}
	 *
	 * @access public
	 * @param
	 *        	string ids string 需要获取数据的微博ID，多个之间用逗号分隔，最多不超过100个
	 * @return array
	 */
	function weibocounts($ids) {
		$params = array ();
		$params ['ids'] = $ids;
		return $this->oauth->get ( 'statuses/counts', $params );
	}
	function get_city($province, $capital) {
		$params = array ();
		$params ['province'] = $province;
		$params ['capital'] = $capital;
		return $this->oauth->get ( 'common/get_city', $params );
	}
	function add_checkin($poiid, $status, $pic, $public) {
		$p = array ();
		$p ['poiid'] = $poiid;
		$p ['status'] = $status;
		$p ['pic'] = $pic;
		...
		return $this->oauth->post ( 'place/pois/add_checkin', $p );
	}
	function logout() {

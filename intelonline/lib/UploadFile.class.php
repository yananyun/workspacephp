...
class UploadFile {
	...
	public $thumbMaxWidth = 330; // 缩略图最大宽度
	public $thumbMaxHeight = 200; // 缩略图最大高度
	...
	public function __construct($savePath = "uploads/", $allowExts = array('gif','jpg','jpeg','bmp','png','txt','zip','swf','csv','xlsx','xls'), $maxSize = 10485760) {
	...
	public function upload($key = '') {
		...
		foreach ( $files as $key => $file ) {
			...
			$file ['savename'] = md5 ( time () . $saveRuleFunc ( $file ['tmp_name'] ) ) . '.' . $file ['extension'];
			$file ['orginal'] = $this->savePath . $file ['savename'];
			if ($this->thumb) {
				$file ['thumbpath'] = $this->thumbPath;
				$file ['thumbname'] = $this->thumbPrefix . $savename;
				$file ['thumbnail'] = $file ['thumbpath'] . $file ['thumbname'];
			}
			// 检查文件类型大小和合法性
			if (! $this->check ( $file )) {
				return false;
			...
			if ($this->autoSave) {
				if (isset ( $this->saeStorage )) {
					$file ['savepath'] = str_replace ( array (
							'../',
							'./' 
					), '', $file ['savepath'] );
	...
	protected function thumb($file) {
		if ($this->thumb && in_array ( $file ['extension'], array (
				'gif',
				'jpg',
				'jpeg',
				'bmp',
			...
			$type = $file ['extension'];
			return Image::thumb ( $imagename, $thumbname, $type, $this->thumbMaxWidth, $this->thumbMaxHeight ); // 生成图像缩略图
		}
		return false;
	}

...
class JsApi_pub extends Common_util_pub {
	 ...
	 */
	function createOauthUrlForMemberInfo() {
	...
	function getOpenid() {
		...
		$this->access_token = $data ['access_token']; // wgs 设置access_token来获取用户详情
		return $this->openid;
	}
	
	/**
	 * 作用：通过curl向微信提交access_token，以获取用户详情
	 * 
	 * @author wgs <gaoshang_s@163.com>
	 */
	function getMemberInfo() {
		$url = $this->createOauthUrlForMemberInfo ();
		// 初始化curl
		$ch = curl_init ();
		// 设置超时
		curl_setopt ( $ch, CURLOP_TIMEOUT, $this->curl_timeout );
	 ...
	 * 作用：设置access_token
	 */
	function setAccessToken($access_) {

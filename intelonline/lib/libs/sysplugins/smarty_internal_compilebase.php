...
abstract class Smarty_Internal_CompileBase {
	...
	public function getAttributes($compiler, $attributes) {
		...
		foreach ( $attributes as $key => $mixed ) {
			...
			if (! is_array ( $mixed )) {
				...
				if (in_array ( $kv ['key'], $this->option_flags )) {
					...
					} else if (is_string ( $kv ['value'] ) && in_array ( trim ( $kv ['value'], '\'"' ), array (
							'true',
							'false' 
					) )) {
					...
					} else if (is_numeric ( $kv ['value'] ) && in_array ( $kv ['value'], array (
							0,
							1 
					) )) {
						if ($kv ['value'] == 1) {
		...
		if ($this->optional_attributes != array (
				'_any' 
		)) {
	...
	public function openTag($compiler, $openTag, $data = null) {
		array_push ( $compiler->_tag_stack, array (
				$openTag,
				$data 
		) );
	}

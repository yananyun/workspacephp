<?php
class newdoll extends Module {
	public function __construct() {
		parent::__construct ( __CLASS__ );
	}
	public function checkLotteryCount($rand) {
		$this->db->tableName = 'newdoll_lottery_conf';
	...
	public function addUser($info) {
		$this->db->tableName = 'newdoll_user';
	...
	public function becomeSi($openid) {
		$sql = "UPDATE newdoll_user SET isSi = 1 WHERE openid = '$openid';";
		// $sql = "UPDATE newdoll_user SET isSi = 1 WHERE openid = '$openid';"."UPDATE newdoll_user SET lastlotterytimes = lastlotterytimes + 1 WHERE openid = '$openid';";
		$this->query ( $sql );
	}
	...
	public function getLastGiftTotal() {
		$this->db->tableName = 'newdoll_gift';
	...
	public function plusLotteryTimes($whereStr) {
		$sql = "UPDATE newdoll_user SET lastlotterytimes = lastlotterytimes + 1 WHERE {$whereStr} ;";
		// $sql = "UPDATE newdoll_user SET lastlotterytimes = 1 WHERE '{$whereStr}' ;";
		$this->query ( $sql );
	}
	...
	public function chkLog($md) {
		$this->db->tableName = 'newdoll_repost_log';
		$count = $this->count ( array (
				'md5' => $md 
		) );
		if ($count > 0) {
	...
	public function getOrderList() {
		...
		$time = time () - 10;
		$sql = "select * from si_queue where time > {$time} order by queuing asc";
	...
	public function duijiang($openid) {
		$this->db->tableName = 'newdoll_gift';
		return $this->first ( "openid='{$openid}' and (type = 1 or type = 3)" );
	}
	public function addMsgLog($data) {
		$this->db->tableName = 'newdoll_msg_log';
